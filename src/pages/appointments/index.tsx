...
export function AppointmentListPage(props: AppointmentListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getAppointments({
        relations: ['user_appointment_user_idTouser', 'user_appointment_medical_staff_idTouser'],
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_user_idTouser',
          header: 'User Appointment User Id Touser',
          accessorKey: 'user_appointment_user_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_user_idTouser?.id}`}
            >
              {record.user_appointment_user_idTouser?.email}
            </Link>
          ),
        }
      : null,
    hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'user_appointment_medical_staff_idTouser',
          header: 'User Appointment Medical Staff Id Touser',
          accessorKey: 'user_appointment_medical_staff_idTouser',
          cell: ({ row: { original: record } }: any) => (
            <Link
              as={NextLink}
              onClick={(e) => e.stopPropagation()}
              href={`/users/view/${record.user_appointment_medical_staff_idTouser?.id}`}
            >
